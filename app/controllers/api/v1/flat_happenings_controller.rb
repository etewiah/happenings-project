class Api::V1::FlatHappeningsController < ApplicationController
  ...
  def index
    ...
    if params[:class]
      classification = params[:class].downcase
    else
      classification = "scrob"
    end

    if params[:day_range]
      start_day = Time.zone.parse(params[:day_range])
      end_day = start_day
    ...
    @flat_happenings = FlatHappening.where(
        :start_time => start_day.beginning_of_day..end_day.end_of_day, 
        :city => city,
